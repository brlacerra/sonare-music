...
export default function BuscaSequencialPage() {
  ...
  async function buscar(nome: string) {
    setCarregando(true);
    try {
      const res = await fetch("/api/algoritmos/sequencial", {
        method: "POST",
  ...
  return (
    <div className="max-w-xl mx-auto mt-20 px-4">
      <h1 className="text-2xl font-bold mb-4 text-white">Busca Sequencial</h1>
      <input
        type="text"
        value={input}
